...
createServer(async (req, res) => {
  ...
  if (pathname === '/index.js' && req.method === 'GET') {
    ...
    createReadStream(resolve('build/server/web/index.js')).pipe(res);
    return;
  }

  if (pathname === '/index.css' && req.method === 'GET') {
    createReadStream(resolve('build/server/web/index.css')).pipe(res);
    return;
  }

  if (pathname === '/' && req.method === 'GET' && req.headers.accept?.includes('text/html')) {
    res.setHeader('Content-Type', 'text/html');
    createReadStream(resolve('build/server/web/index.html')).pipe(res);
