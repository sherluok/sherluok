...
type EventStyle = (typeof EventStyle)[keyof (typeof EventStyle)];
...
const emitApiEvent = (event: P.Event, domainName: string, modulePrefix: string, eventStyle: EventStyle) => {
  ...
  switch (eventStyle) {
    case EventStyle.REGULAR: {
      ...
      const params = event.parameters ? `${prefix}${toEventPayloadName(event.name)}` : 'void';
      emitLine(`${event.name}: Observable<${params}>;`);
      break;
    }
    case EventStyle.INSPECTOR_PROTOCOL_TESTS: {
...
const emitApi = (moduleName: string, protocolModuleName: string, domains: P.Domain[], eventStyle = EventStyle.REGULAR) => {
  ...
  emitHeaderComments()
  emitLine(`import { Protocol } from './${protocolModuleName}';`);
  emitLine(`import { Observable } from 'rxjs';`);
  emitLine()
  ...
  emitLine()
  domains.forEach(d => {
    emitLine(`${d.domain}: ${moduleName}.${d.domain}Api;`)
    emitLine()
  });
  emitCloseBlock()
  emitLine()
...
(async function main() {
  // TODO: validate via Zod against protocol-schema.d.ts
  console.log(`Downloading js_protocol.json...`);
  const jsProtocol = await fetch('https://raw.githubusercontent.com/ChromeDevTools/devtools-protocol/refs/heads/master/json/js_protocol.json').then((res) => res.json()) as IProtocol;
  console.log(`Downloading browser_protocol.json...`);
  const browserProtocol = await fetch('https://raw.githubusercontent.com/ChromeDevTools/devtools-protocol/refs/heads/master/json/browser_protocol.json').then((res) => res.json()) as IProtocol;
  const protocolDomains: P.Domain[] = jsProtocol.domains.concat(browserProtocol.domains);

  const destProtocolFilePath = path.join(__dirname, '../src/types/protocol.d.ts');
  ...
  const destApiFilePath = path.join(__dirname, '../src/types/protocol-session.d.ts');
  const apiModuleName = 'ProtocolSession';
